...
module Make (M : Monad) = struct
  type ('a, 'kind) parser = 'kind context -> 'kind -> ('a * 'kind) M.t

  type 'a t = ('a, values) parser

  type 'a fields_parser = ('a, Fields.t) parser

  let return x _ctx state = M.return (x, state)
  ...
  let loc : type k. (Loc.t, k) parser =
   fun ctx state ->
    match ctx with
    | Values (loc, _, _) -> M.return (loc, state)
    | Fields (loc, _, _) -> M.return (loc, state)

  let eos : type k. (bool, k) parser =
   fun ctx state ->
    match ctx with
    | Values _ -> M.return (state = [], state)
    | Fields _ -> M.return (Name.Map.is_empty state.unparsed, state)
  ...
  let liftM t context sexps = M.return (t context sexps)
  ...
  let set_input : type k. ast list M.t -> (unit, k) parser =
   fun sexps context _ ->
    let open M.O in
    let+ sexps = sexps in
    (match context with
  ...
  let next_with_user_context f ctx sexps =
    ...
    | [] -> end_of_list ctx
    | sexp :: sexps ->
      let+ x = f (get_user_context ctx) sexp in
      (x, sexps)
    [@@inline always]
  ...
  let peek _ctx sexps =
    match sexps with
    | [] -> M.return (None, sexps)
    | sexp :: _ -> M.return (Some sexp, sexps)
    [@@inline always]

  let peek_exn ctx sexps =
    match sexps with
    | [] -> end_of_list ctx
    | sexp :: _ -> M.return (sexp, sexps)
    [@@inline always]

  let junk = next (fun _ -> M.return ())

  let junk_everything : type k. (unit, k) parser =
   fun ctx state : (unit * k) M.t ->
    match ctx with
    | Values _ -> M.return ((), [])
    | Fields _ -> M.return ((), Fields.junk_unparsed state)

  let keyword kwd =
    next (function
      | Atom (_, A s) when s = kwd -> M.return ()
      | sexp ->
  ...
  let either =
    (* Before you read this code, close your eyes and internalise the fact that
       this code is temporary. It is a temporary state as part of a larger work
       to turn [Decoder.t] into a pure applicative. Once this is done, this
       function will be implemented in a better way and with a much cleaner
       semantic. *)
  ...
  let field name ?default ?on_dup t (Fields (loc, _, uc)) state =
    ...
    | Some { values; entry; _ } ->
      ...
      let+ x = t ctx values in
      let x = result ctx x in
      (x, Fields.consume state name)
    | None -> (
      match default with
      | Some v -> M.return (v, Fields.add_known state name)
  ...
  let field_o name ?on_dup t (Fields (_, _, uc)) state =
    ...
    | None -> M.return (None, Fields.add_known state name)
  ...
  let fields t (Values (loc, cstr, uc)) sexps =
    ...
    let+ x = t ctx (Fields.of_values sexps) in
    let x = result ctx x in
    (x, [])
  ...
  let leftover_fields_generic t more_fields (Fields (loc, cstr, uc)) state =
    ...
    let+ x =
      ...
      let+ x = repeat t ctx (Fields.unparsed_ast state) in
      result ctx x
    in
  ...
  let kind : type k. (kind, k) parser =
   fun ctx state ->
    match ctx with
    | Values (loc, cstr, _) -> M.return (Values (loc, cstr), state)
    | Fields (loc, cstr, _) -> M.return (Fields (loc, cstr), state)
  ...
  let fields_mutually_exclusive ?on_dup ?default fields
    ...
    | [] -> (
      ...
      | Some default -> M.return (default, state))
    | [ (_name, res) ] -> M.return (res, state)
    | _ :: _ :: _ as results ->
      let names = List.map ~f:fst results in
      fields_mutual_exclusion_violation loc names
end

include Make (Monad.Id)
